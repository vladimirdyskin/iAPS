...
public struct BolusNormalPumpEvent: TimestampedPumpEvent {
    ...
    public init(
        ...
        unabsorbedInsulinRecord: UnabsorbedInsulinPumpEvent?,
        amount: Double,
        programmed: Double,
        unabsorbedInsulinTotal: Double,
        type: BolusType,
        duration: TimeInterval,
        wasRemotelyTriggered: Bool
    ) {
        self.length = length
    ...
    public init?(availableData: Data, pumpModel: PumpModel) {
        func doubleValueFromData(at index: Int) -> Double {
            Double(availableData[index])
        }

        func decodeInsulin(from bytes: Data) -> Double {
            Double(Int(bigEndianBytes: bytes)) / Double(pumpModel.insulinBitPackingScale)
        }
        ...
        if pumpModel.larger {
            ...
            wasRemotelyTriggered = availableData[11] & 0b0100_0000 != 0
            ...
            wasRemotelyTriggered = availableData[7] & 0b0100_0000 != 0
